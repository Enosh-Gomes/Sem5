...
void worstFitAllocate(SegmentRegion region[], int n, int process_size, vector<string>& logs) {
    int worstIdx = -1;
    int maxRemaining = -1;

    for (int i = 0; i < n; i++) {
        if (region[i].free_space >= process_size) {
            if (region[i].free_space > maxRemaining) {
                maxRemaining = region[i].free_space;
                worstIdx = i;
            }
        }
    }
    ...
    if (worstIdx != -1) {
        region[worstIdx].segments.push_back(process_size);
        region[worstIdx].free_space -= process_size;

        string log = "Process of size " + to_string(process_size)
                    + " KB allocated in segment of size "
                    + to_string(region[worstIdx].max_space) + " KB";
        logs.push_back(log);
    } else {
...
int main() {
    ...
    for (Process p : process)
        worstFitAllocate(region, n, p.process_size, logs);
