...
void firstFitAllocate(SegmentRegion region[], int n, int process_size, vector<string>& logs) {
    bool allocated = false;

    for (int i = 0; i < n; i++) {
        if (region[i].free_space >= process_size) {
            region[i].segments.push_back(process_size);
            region[i].free_space -= process_size;

            string log = "Process of size " + to_string(process_size)
                        + " KB allocated in segment of size "
                        + to_string(region[i].max_space) + " KB";
            logs.push_back(log);
            allocated = true;
            break;
        }
    ...
    if (!allocated) {
        string log = "No sufficient space available for process of size "
                    + to_string(process_size) + " KB";
        logs.push_back(log);
    }
}
...
int main() {
    ...
    for (Process p : process)
        firstFitAllocate(region, n, p.process_size, logs);
