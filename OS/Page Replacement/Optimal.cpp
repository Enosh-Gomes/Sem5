...
void OptimalPageReplacement(vector<string> reference_string, vector<string>& frame, int frames) {
    int page_faults = 0;
    ...
    for(int i = 0; i < reference_length; i++) {
        string current_page = reference_string[i];
        bool page_hit = false;
        ...
        if(!page_hit) {
            ...
            if(empty_index != -1) {
                frame[empty_index] = current_page;
            } else {
                int farthest = i + 1;
                int replace_index = -1;
                for(int j = 0; j < frames; j++) {
...
int main() {
    ...
    OptimalPageReplacement(reference_string, frame, frames);
